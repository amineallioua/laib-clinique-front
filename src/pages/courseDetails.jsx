...
const Details = () => {
  ...
  if (!course) {
    return <p className='flex items-center justify-center text-center font-extrabold text-[40px] text-black my-[200px]'>
      Course not found <br />
      <PiEmptyBold className='text-center font-extrabold text-[80px] text-black ml-10' />
    </p>; // Handle case when course is null or not found
  }
  ...
  return (
    ...
    >
      <div className='max-w-[96%] shadow-xl shadow-slate-300 py-5 pb-10 px-5 flex flex-col lg:flex-row items-center bg-[#d4f5ff] rounded-[50px] mx-auto'>
        ...
        <div className={`w-full flex transition-opacity duration-300 ease-in-out ${isOpen1 ? 'opacity-0 pointer-events-none' : 'opacity-100'} left-0 justify-center bg-transparent backdrop-blur-sm h-full p-[10%] fixed top-0 z-40`} onClick={() => { toggleMenu1(); navigate('/courses'); }}>
          ...
          >
            <center>
              ...
              <h1 className='text-[22px] font-extrabold text-[#374885]'>Enrollment request submitted <br /> Await our call</h1>
            </center>
            <button>
